...
// Keep to manageable levels by undoing all divisions
const productOfAllConditions = monkeys
  .map(({ test }) => test.condition)
  .reduce((acc, divBy) => acc * divBy);
...
function calculateWorryLevel(operation: MonkeyOperation, item: bigint): bigint {
  ...
  // Keep to manageable levels by taking the modulus of the level with the conditional test product
  return newWorryLevel % productOfAllConditions;
}
...
for (const _round of [...Array(10_000).keys()].map((r) => r + 1)) {
...
const [first, second] = monkeys.map((m) => m.numInspections).sort((a, b) =>
  b - a
);
